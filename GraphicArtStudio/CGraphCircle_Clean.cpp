...
void CGraphCircle::DrawMidpointCircle(CDC* pDC)
{
	...
	{
		pDC->Ellipse(m_center.x - m_radius, m_center.y - m_radius, m_center.x + m_radius + 1, m_center.y + m_radius + 1);
	}
...
{
	double x = (double)m_center.x;
	double y = (double)m_center.y;
	m_center.x = (int)(matrix[0][0] * x + matrix[0][1] * y + matrix[0][2]);
	m_center.y = (int)(matrix[1][0] * x + matrix[1][1] * y + matrix[1][2]);
...
{
	return CRect(m_center.x - m_radius - 5, m_center.y - m_radius - 5, m_center.x + m_radius + 5, m_center.y + m_radius + 5);
}
